# -*- coding: utf-8 -*-
# Leonard Aye --- 2019
# zg2uni-file-converter.py using Rabbit-Python function module from
...
def convert_file(zgfile,inpath,outpath,namext):
    # Read in the input file in ZG format
    with open(inpath+'\\'+zgfile, encoding="utf-8") as f:
        try:
            zlines = f.read().splitlines()
            print('Converting: ', zgfile)
            convert_each_line(zgfile, zlines, outpath, namext)
        except UnicodeDecodeError:
            print('Skipping: ', zgfile)
    f.close()

def convert_each_line(zgfile,zlines,outpath,namext):
    #generate a new file path, name, namext and extension
    unifile=outpath+'\\'+zgfile[:-4]+'.'+namext+'.'+zgfile[-3:]
    u = open(unifile, "w+", encoding="utf-8")

    #convert every line where ZG text
    for zline in zlines:
        #print('start: ', zline)
        if len(zline) != 0: # not an empty line
            uline = zg2uni(zline)
            u.write(uline + '\n')
#            firstchar =ord(zline[0])
#            if firstchar != 65279: #i.e. not a spurious character
#                uline  = zg2uni(zline)
#                u.write(uline+'\n')
#            else:
#                u.write(zline+'\n')
...
def main(inpath,outpath,extension,namext):
    print(datetime.datetime.now())
    for filename in os.listdir(inpath):
        # Convert file if file extension match the given extension
        if extension != '' and filename[-3:] == extension:
            convert_file(filename, inpath, outpath, namext)
        elif extension == '':  # Convert file if extension if not provided, i.e. convert all files
            convert_file(filename, inpath, outpath, namext)
    print('End of file conversion to Unicode')
    print(datetime.datetime.now())


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Converts Zawgyi text file to Unicode.')
    ...
    parser.add_argument('-n', "--namext", default='', help='File name extension to be added, e.g. uni, mm')
    args = parser.parse_args()
    main(args.source, args.dest, args.extension, args.namext)
